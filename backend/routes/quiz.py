...
def _build_topic_fallback(topic: str, difficulty_level: int) -> List[Dict[str, Any]]:
    ...
    questions = [
        ...
        },
        {
            ...
            "correct": 0,
            "explanation": f"Meaningful evaluation in {topic} focuses on {terms['evaluation']}.",
        },
        {
            "id": 5,
...
STOPWORDS = {
    ...
    "on", "with", "by", "as", "at", "this", "that", "these", "those", "it",
    "from", "be", "was", "were", "can", "could", "should", "would", "about",
    "into", "their", "there", "them", "such", "also", "than", "then",
    "using", "used", "use",
}
...
def _build_content_fallback(content: str, difficulty_level: int) -> List[Dict[str, Any]]:
    ...
    return [
        {
            "id": 1,
            "question": "Based on the provided content, what is the most likely main focus?",
            "options": [
        ...
        {
            "id": 4,
            "question": "Which inference is most reasonable from the provided content?",
            "options": [
...
def _build_fallback_questions(
    topic: str, content: str, difficulty_level: int
) -> List[Dict[str, Any]]:
    if topic:
...
async def _generate_quiz(
    ...
    questions = await _generate_with_gemini(
        topic=topic, content=content, difficulty_level=difficulty_level
    )
    source = "gemini"
...
async def submit_quiz(req: QuizSubmitRequest):
    ...
    for q in req.questions:
        if len(q.options) != 4:
            raise HTTPException(
                status_code=400, detail=f"Question {q.id} must have exactly 4 options."
            )
        if q.correct < 0 or q.correct > 3:
            raise HTTPException(
                status_code=400, detail=f"Question {q.id} has invalid correct index."
            )
